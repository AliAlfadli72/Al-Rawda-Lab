    ...
    <div className="font-sans antialiased text-primary bg-white" dir="rtl">
      ...
      <section className="relative pt-32 pb-24 bg-primary overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(#a0843c_1px,transparent_1px)] [background-size:20px_20px]"></div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          ...
          <p className="max-w-2xl mx-auto text-white/60 text-lg md:text-xl font-light">
      ...
      <section className="py-24 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 gap-8">
            {services.map((s, i) => (
              <div key={i} className="group p-1 bg-white rounded-[2rem] border border-primary/10 hover:border-gold/50 transition-all duration-500 shadow-sm hover:shadow-2xl hover:-translate-y-1">
                <div className="p-8 md:p-10">
                  <div className="w-16 h-16 bg-primary text-gold rounded-2xl flex items-center justify-center text-3xl mb-8 group-hover:scale-110 transition-transform">
                    {s.icon}
                  </div>
                  <h3 className="text-2xl font-bold mb-4 group-hover:text-gold transition-colors text-primary">{s.title}</h3>
                  <p className="text-primary/60 leading-relaxed mb-8">
                    {s.desc}
                  </p>
                  <div className="flex flex-wrap gap-3">
                    {s.features.map((f, idx) => (
                      <span key={idx} className="flex items-center gap-2 text-sm font-semibold text-primary/60 bg-primary/5 px-4 py-1.5 rounded-full">
      ...
      {/* ================= TECHNICAL WORKFLOW ================= */}
      <section className="py-24 bg-primary/5 px-6 overflow-hidden">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-primary mb-4">آلية العمل الرقمية</h2>
            <p className="text-primary/60">نظام صارم لضمان الجودة من الاستلام حتى التسليم</p>
          </div>
          ...
          <div className="relative">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-8 relative z-10">
              {[
                ...
                <div key={i} className="bg-white p-8 rounded-2xl text-center shadow-sm border border-primary/10 group hover:border-gold transition-all">
                  <span className="block text-4xl font-black text-gold/20 group-hover:text-gold/40 mb-4 transition-colors">
                    {step.step}
                  </span>
                  <h4 className="font-bold text-primary mb-2">{step.label}</h4>
                  <p className="text-xs text-primary/50">{step.detail}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="py-24 px-6 text-center">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl md:text-4xl font-bold mb-8 text-primary">جاهزون للارتقاء بجودة عيادتك؟</h2>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-6">
            <Link
              to="/doctors"
              className="bg-gold text-white px-12 py-4 rounded-xl font-bold text-lg shadow-xl shadow-gold/20 hover:bg-[#b8963f] hover:scale-105 transition-all"
            >
              ابدأ التعاون الآن
            </Link>
            <Link
              to="/contact"
              className="flex items-center justify-center gap-3 bg-primary text-white px-12 py-4 rounded-xl font-bold text-lg hover:bg-primary/90 transition-all"
            >
